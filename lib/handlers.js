...
handlers._tokens.post = function(data, callback){
    ...
    if(phone && password){
        _performance.mark('User lookup started');
        _data.read('users', phone, function(err, userData){
            _performance.mark('User lookup completed');
            if(!err){

                // Hash the password
                _performance.mark('begining hashpassword parsing');
                var hashedPassword = helpers.hash(password); 
                _performance.mark('hashpassword parsing completed');
                if(hashedPassword == userData.hashedPassword){
                    ...
                    _data.create('tokens', tokenId, tokenObject, function(err){
                        ...
                        _performance.measure('Bengining to end','entered function', 'storing token complete');
                        _performance.measure('Validating user input', 'entered function', 'Inputs Validated');
                        _performance.measure('User lookup', 'User lookup started', 'User lookup completed');
                        _performance.measure('Password Hashing', 'begining hashpassword parsing', 'hashpassword parsing completed');
